...
function cfg()
{
    ...
    if (!file_exists($p)) {
        out([
            'status' => 'error',
            'message' =>
                'أنشئ public/config.php من config.example.php أولاً.'
        ], 500);
    }
...
";
    ...
    $headers[] =
        'From: Youmi <noreply@youmi.wuaze.com>';

    $headers[] =
        'Reply-To: noreply@youmi.wuaze.com';
...
function install()
{
    ...
    $sql = [
        ...
        "CREATE TABLE IF NOT EXISTS users (
            ...
            role ENUM('buyer','merchant','admin')
    ...
    // Safe migration for installations created before trial_days existed.
    try {
        $columns = $p->query(
            "SHOW COLUMNS FROM subscription_plans LIKE 'trial_days'"
        )->fetchAll();
        ...
        if (!$columns) {
            ...
            $p->exec(
                "UPDATE subscription_plans
                 SET trial_days = CASE
                    WHEN type='free' THEN 30
                    ELSE 0
                 END"
            );
        }
    } catch (Throwable $e) {
        // Keep installation compatible; the API will report a useful error if the column is unavailable.
    }
...
function tenantDb($storeId)
{
    ...
    try {
        ...
        out([
            'status' => 'error',
            'message' =>
                'تعذر الاتصال بقاعدة بيانات المتجر.',
            'debug' =>
                $e->getMessage()
        ], 500);
    }
...
function getSubscriptionPlan($id)
{
    ...
    $plan['trial_days'] =
        isset($plan['trial_days'])
            ? max(0, (int)$plan['trial_days'])
            : 0;
...
function defaultSubscriptionPlan()
{
    $q = db()->query(
    ...
    $plan = $q->fetch();
    return $plan ? getSubscriptionPlan($plan['id']) : null;
}
...
if (
    ...
    $storeCount =
        (int)($qOwner->fetch()['total'] ?? 0);

    $plan =
        merchantSubscriptionPlan($u['id'])
    ...
    if ($storeCount >= $maxStores) {
        out([
            'status' => 'error',
            'message' =>
                'لقد وصلت إلى الحد الأقصى للمتاجر المسموح به في خطتك (' .
                $maxStores .
                ').'
        ], 400);
    }
    ...
    if (!$plan) {
        out([
            'status' => 'error',
            'message' => 'لا توجد خطة اشتراك افتراضية مفعلة.'
        ], 400);
    }

    $now = date('Y-m-d H:i:s');
    $trialDays = (int)($plan['trial_days'] ?? 0);
    $durationDays = $trialDays > 0 ? $trialDays : (int)$plan['duration_days'];
    $expiresAt = date('Y-m-d H:i:s', time() + ($durationDays * 86400));
    $isTrial = $trialDays > 0;

    $s['subscription'] = array_merge(
        $s['subscription'] ?? [],
        [
            'planId' => $plan['id'],
            'planName' => $plan['name'],
            'planType' => $plan['type'],
            'priceDzd' => (float)$plan['price_dzd'],
            'durationDays' => $durationDays,
            'maxStores' => (int)$plan['max_stores'],
            'maxProducts' => $plan['max_products'] === null ? null : (int)$plan['max_products'],
            'status' => $isTrial ? 'active_trial' : 'subscribed',
            'startedAt' => $now,
            'expiresAt' => $expiresAt,
            'isTrialActive' => $isTrial,
            'trialDaysLeft' => $isTrial ? $trialDays : 0,
            'isActive' => true
        ]
    );
...
if (
    ...
    /*
    | Important:
    | Admin must NOT call merchantStore()
    | because merchantStore() only accepts merchant role.
    */
    ...
    if ($i < 0) {
        ...
        $plan = $planId ? getSubscriptionPlan($planId) : null;
        $maxProducts = $plan
            ? $plan['max_products']
            : null;

        if ($maxProducts !== null && count($s['products']) >= (int)$maxProducts) {
            out([
                'status' => 'error',
                'message' =>
                    'لقد وصلت إلى الحد الأقصى للمنتجات المسموح به في خطتك (' .
                    (int)$maxProducts .
                    ').'
            ], 400);
        }
...
if (
    ...
    $isActive =
        array_key_exists('isActive', $plan)
            ? (!empty($plan['isActive']) ? 1 : 0)
            : (array_key_exists('active', $plan)
...
if (
    ...
    */
    ...
    $planTrialDays = max(0, (int)($plan['trial_days'] ?? 0));
    $requestedTrial = !empty($d['isTrial']);
    $isTrial = $requestedTrial || ($plan['type'] === 'free' && $planTrialDays > 0);
    $durationDays = $isTrial && $planTrialDays > 0
        ? $planTrialDays
        : (int)$plan['duration_days'];
