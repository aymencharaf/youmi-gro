      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white text-xs py-2 px-4 lg:px-8 border-b border-indigo-900/50 shadow-inner">
        <div className="max-w-[1400px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-2.5">
          ...
          <div className="flex items-center gap-3 text-xs font-semibold">
            <button onClick={onOpenInfinityFreeModal} className="flex items-center gap-1.5 text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 px-2.5 py-1 rounded-lg transition hidden md:flex">
      ...
      <section className="max-w-[1400px] mx-auto px-4 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          <div className="lg:col-span-8 bg-slate-950 rounded-3xl overflow-hidden shadow-xl border border-slate-800 relative min-h-[380px] flex flex-col justify-between p-6 md:p-10 text-white">
            <img src={heroSlides[activeHeroSlide].image} alt="Hero Banner" className="absolute inset-0 w-full h-full object-cover opacity-25 mix-blend-overlay" />
            ...
            <div className="relative z-10 flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-slate-300 bg-slate-900/60 backdrop-blur-md px-3 py-1 rounded-xl border border-slate-700">58 ولاية شحن ومتابعة</span>
            </div>
          ...
          <div className="lg:col-span-4 flex flex-col justify-between gap-4">
            ...
            <div className="p-5 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                ...
                <div className="p-2.5 bg-slate-50 rounded-2xl border border-slate-100"><p className="font-extrabold text-emerald-600 text-sm font-mono">58 ولاية</p><p className="text-[10px] text-slate-500 font-medium">شحن وتوصيل</p></div>
                <div className="p-2.5 bg-slate-50 rounded-2xl border border-slate-100"><p className="font-extrabold text-amber-600 text-sm font-mono">100% COD</p><p className="text-[10px] text-slate-500 font-medium">دفع عند الاستلام</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="suppliers" className="max-w-[1400px] mx-auto px-4 lg:px-8 py-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            ...
            <h2 className="text-xl md:text-2xl font-black text-slate-900 font-['Cairo']">متاجر وموردو الجملة المعتمدون بالجزائر</h2>
          </div>
          <button
            ...
            <div key={s.id} className="bg-white border border-slate-200 hover:border-indigo-400 rounded-3xl p-5 shadow-xs hover:shadow-md transition space-y-4 flex flex-col justify-between group">
              <div className="space-y-3">
                ...
                <div className="pt-2 border-t border-slate-100 grid grid-cols-2 gap-2 text-[11px]">
                  <div className="p-2 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-1.5 text-slate-700"><Truck className="w-3.5 h-3.5 text-indigo-600 shrink-0" /><span className="truncate font-semibold">شحن 58 ولاية (Yalidine)</span></div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-xs space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
              ...
              <h2 className="text-xl md:text-2xl font-black text-slate-900 font-['Cairo']">منتجات الجملة المتاحة للطلب الآن</h2>
            </div>
            <div className="flex items-center gap-2 overflow-x-auto py-1">
      ...
      <section className="max-w-[1400px] mx-auto px-4 lg:px-8 py-8">
        <div className="bg-slate-900 text-white rounded-3xl p-8 shadow-xl border border-slate-800 space-y-8">
          <div className="text-center max-w-2xl mx-auto space-y-2">
            ...
            <p className="text-xs text-slate-400">نوفر البيئة الآمنة والتكنولوجية الأحدث لتسريع مبايعات الجملة والشحن المباشر لـ 58 ولاية.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="p-5 bg-slate-800/60 rounded-2xl border border-slate-700 space-y-3"><div className="w-10 h-10 rounded-xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center border border-emerald-500/30"><ShieldCheck className="w-5 h-5" /></div><h3 className="text-sm font-bold text-white font-['Cairo']">1. الدفع عند الاستلام 100% (COD)</h3><p className="text-xs text-slate-400 leading-relaxed">سداد قيمة طلبية الجملة نقداً لمندوب الشحن عند المعاينة والاستلام في عنوان التاجر دون مخاطرة الدفع المسبق.</p></div>
            ...
            <div className="p-5 bg-slate-800/60 rounded-2xl border border-slate-700 space-y-3"><div className="w-10 h-10 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center border border-amber-500/30"><Gift className="w-5 h-5" /></div><h3 className="text-sm font-bold text-white font-['Cairo']">3. تجربة مجانية 30 يوماً</h3><p className="text-xs text-slate-400 leading-relaxed">استمتع بـ 30 يوماً مجاناً لتجربة كافة أدوات المتجر والذكاء الاصطناعي، ثم التجديد بـ 3,500 دج شهرياً عبر بريدي موب.</p></div>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex justify-end dir-rtl">
          <div className="bg-white w-full max-w-md h-full shadow-2xl p-6 flex flex-col justify-between space-y-4">
            ...
            {cartItems.length > 0 && <div className="pt-4 border-t border-slate-200 space-y-3"><div className="flex items-center justify-between text-xs font-bold text-slate-800"><span>المجموع الإجمالي للطلبية:</span><span className="text-base text-rose-600 font-mono font-black">{totalCartPriceDzd.toLocaleString()} دج</span></div><div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200 text-[11px] text-emerald-900 flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" /><span>طريقة السداد: الدفع نقداً عند استلام شحنة الجملة (COD)</span></div><button onClick={() => { if (cartItems.length > 0) { onSelectStore(cartItems[0].store, 'STORE_FRONT'); setIsCartOpen(false); } }} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-md transition flex items-center justify-center gap-2"><span>متابعة إتمام الطلب بالمتجر المختص</span><ArrowRight className="w-4 h-4" /></button></div>}
          </div>
        </div>
      )}
      ...
      <footer className="bg-slate-950 text-slate-400 text-xs py-10 px-4 lg:px-8 border-t border-slate-800 mt-12">
        <div className="max-w-[1400px] mx-auto space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="space-y-2"><h4 className="text-xs font-bold text-white font-['Cairo']">روابط الاستضافة والدعم</h4><ul className="space-y-1.5 text-slate-400"><li><button onClick={() => onNavigate('CREATE_STORE')} className="hover:text-amber-400 transition">أنشئ متجر جملة (30 يوماً مجاناً)</button></li><li><a href="#suppliers" className="hover:text-amber-400 transition">دليل الموردين والمصانع</a></li><li><button onClick={currentMember?.role === 'merchant' ? onOpenMerchantDashboard : onOpenLoginModal} className="hover:text-amber-400 transition">{currentMember?.role === 'merchant' ? 'لوحة تحكم البائع' : 'لوحة تحكم التجّار'}</button></li><li><button onClick={onOpenInfinityFreeModal} className="text-emerald-400 font-bold hover:underline transition flex items-center gap-1">🌐 دليل الرفع على InfinityFree</button></li></ul></div>
            <div className="space-y-2"><h4 className="text-xs font-bold text-white font-['Cairo']">الشحن والدفع</h4><ul className="space-y-1.5 text-slate-400"><li className="flex items-center gap-1.5"><Truck className="w-3.5 h-3.5 text-indigo-400" /><span>شحن 58 ولاية عبر Yalidine API</span></li><li className="flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /><span>الدفع عند الاستلام 100% (COD)</span></li><li className="flex items-center gap-1.5"><CreditCard className="w-3.5 h-3.5 text-amber-400" /><span>سداد الاشتراكات عبر BaridiMob</span></li></ul></div>
            <div className="space-y-2"><h4 className="text-xs font-bold text-white font-['Cairo']">الدعم والمساعدة</h4><p className="text-xs text-slate-400">فريق خدمة العملاء متواجد لمساعدتك في إنشاء وافتتاح متجرك بالجملة.</p><p className="font-mono text-amber-400 font-bold">+213 (0) 550 12 34 56</p></div>
          </div>
          <div className="pt-6 border-t border-slate-900 flex flex-col sm:flex-row items-center justify-between gap-4 text-slate-500 text-[11px]"><p>جميع الحقوق محفوظة © 2026 لمنصة يومي لتجارة الجملة بالجزائر (Youmi Market DZ)</p><div className="flex items-center gap-4"><span>🇩🇿 تغطية كاملة لـ 58 ولاية</span><span>•</span><span>100% دفع عند الاستلام</span></div><button type="button" onClick={onOpenAdminLoginModal} aria-label="إدارة المنصة" title="إدارة المنصة" className="text-[9px] text-slate-700/40 hover:text-slate-400 transition px-1 py-0.5">إدارة</button></div>
        </div>
      </footer>
    </div>
  );
