...
// رقم واتساب فريق مساعدة منصة Youmi (يُعرض في أسفل الصفحة الرئيسية)
// يمكن تغييره لاحقاً إلى رقم الدعم الرسمي بصيغة جزائرية أو دولية.
const YOUMI_SUPPORT_WHATSAPP = '213669964145';
const YOUMI_SUPPORT_MESSAGE = 'السلام عليكم، أحتاج المساعدة في مشكلة واجهتني في منصة يومي للتجارة والجملة.';

const WhatsAppBrandIcon: React.FC<{ className?: string }> = ({ className = 'w-5 h-5' }) => (
  <svg viewBox="0 0 24 24" aria-hidden="true" className={className} fill="currentColor">
    <path d="M20.52 3.48A11.87 11.87 0 0 0 12.05 0C5.5 0 .17 5.33.17 11.89c0 2.09.55 4.13 1.59 5.92L.08 24l6.34-1.66a11.9 11.9 0 0 0 5.63 1.43h.01c6.55 0 11.88-5.33 11.88-11.88 0-3.18-1.24-6.16-3.42-8.41ZM12.06 21.77h-.01a9.9 9.9 0 0 1-5.04-1.38l-.36-.21-3.76.98 1-3.66-.23-.38a9.86 9.86 0 0 1-1.51-5.24c0-5.44 4.43-9.87 9.88-9.87 2.63 0 5.1 1.03 6.96 2.9a9.83 9.83 0 0 1 2.89 6.98c0 5.44-4.43 9.88-9.82 9.88Zm5.42-7.4c-.3-.15-1.77-.87-2.04-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.17-.17.2-.35.22-.65.07-.3-.15-1.26-.46-2.4-1.48-.89-.79-1.49-1.77-1.66-2.07-.17-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.07-.15-.67-1.62-.92-2.22-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37-.27.3-1.03 1.01-1.03 2.46s1.06 2.85 1.21 3.05c.15.2 2.08 3.18 5.04 4.46.7.3 1.25.48 1.68.62.71.23 1.35.2 1.86.12.57-.08 1.77-.72 2.02-1.42.25-.7.25-1.3.17-1.42-.07-.12-.27-.2-.57-.35Z" />
  </svg>
);
      ...
      <footer className="bg-slate-950 text-slate-400 text-xs py-10 px-4 lg:px-8 border-t border-slate-800 mt-12">
        <div className="max-w-[1400px] mx-auto space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-white font-['Cairo'] flex items-center gap-2"><Headphones className="w-4 h-4 text-emerald-400" /><span>الدعم والمساعدة</span></h4>
              <p className="text-xs text-slate-400 leading-relaxed">واجهت أي مشكلة في التسجيل، الطلب، الدفع، الشحن أو استخدام المنصة؟ تواصل مباشرة مع فريق مساعدة Youmi.</p>
              <a
                href={`https://wa.me/${YOUMI_SUPPORT_WHATSAPP}?text=${encodeURIComponent(YOUMI_SUPPORT_MESSAGE)}`}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="التواصل مع فريق مساعدة Youmi عبر واتساب"
                className="w-full inline-flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-white font-black text-xs shadow-lg shadow-emerald-950/20 transition-transform hover:-translate-y-0.5"
              >
                <WhatsAppBrandIcon className="w-5 h-5" />
                <span>تواصل مع فريق المساعدة عبر واتساب</span>
              </a>
            </div>
          </div>
          <div className="rounded-2xl border border-emerald-500/20 bg-emerald-950/20 p-3 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-emerald-100 text-xs font-bold">
              <WhatsAppBrandIcon className="w-5 h-5 text-emerald-400" />
              <span>فريق Youmi متواجد لمساعدتك عند مواجهة أي مشكلة.</span>
            </div>
            <a
              href={`https://wa.me/${YOUMI_SUPPORT_WHATSAPP}?text=${encodeURIComponent(YOUMI_SUPPORT_MESSAGE)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500 text-emerald-300 hover:text-white border border-emerald-500/30 font-bold text-[11px] transition"
            >
