...
export const ProductsTab: React.FC<ProductsTabProps> = ({ store, onUpdateStore }) => {
  ...
  const openAddModal = () => {
    ...
    setTierPrices([{ minQuantity: 10, price: 3300 }, { minQuantity: 50, price: 3000 }]);
    setVariants([]);
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (product: Product) => {
    ...
    setTierPrices((product.tierPrices || []).map((x) => ({ ...x })));
    setVariants((product.variants || []).map((x) => ({ ...x, options: [...x.options] })));
    setIsModalOpen(true);
  };
  ...
  const handleSaveProduct = (e: React.FormEvent) => {
    ...
    const productToSave: Product = {
      ...
      badge,
      isAvailable: stock > 0,
      lowStockThreshold: Math.max(0, lowStockThreshold || 0),
      tierPrices: tierPrices
        .filter((x) => Number(x.minQuantity) > 0 && Number(x.price) > 0)
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-emerald-50/60 border border-emerald-100 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <label className="block text-xs font-black text-emerald-950">أسعار حسب الكمية</label>
                    <p className="text-[10px] text-emerald-800 mt-0.5">مثال: 10 قطع بسعر، و50 قطعة بسعر أقل.</p>
                  </div>
                  <button type="button" onClick={() => setTierPrices((p) => [...p, { minQuantity: 10, price: Math.max(1, price) }])} className="px-2.5 py-1.5 rounded-lg bg-white border border-emerald-200 text-emerald-700 text-[10px] font-bold flex items-center gap-1">
                    <PlusCircle className="w-3.5 h-3.5" /> إضافة سعر
                  </button>
                </div>
                {tierPrices.map((tier, index) => (
                  <div key={index} className="grid grid-cols-[1fr_1fr_auto] gap-2 items-end">
                    <div><label className="block text-[10px] font-bold text-slate-600 mb-1">من كمية</label><input type="number" min={1} value={tier.minQuantity} onChange={(e) => setTierPrices((p) => p.map((x,i)=>i===index?{...x,minQuantity:Number(e.target.value)}:x))} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs" /></div>
                    <div><label className="block text-[10px] font-bold text-slate-600 mb-1">السعر للوحدة</label><input type="number" min={1} value={tier.price} onChange={(e) => setTierPrices((p) => p.map((x,i)=>i===index?{...x,price:Number(e.target.value)}:x))} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs" /></div>
                    <button type="button" onClick={() => setTierPrices((p) => p.filter((_,i)=>i!==index))} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
              ...
              <div className="p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div><label className="block text-xs font-black text-indigo-950">خيارات المنتج / Product Variations</label><p className="text-[10px] text-indigo-800 mt-0.5">مثل اللون، المقاس، النوع. اكتب الخيارات مفصولة بفاصلة.</p></div>
                  <button type="button" onClick={() => setVariants((p) => [...p, { name: 'اللون', options: [''] }])} className="px-2.5 py-1.5 rounded-lg bg-white border border-indigo-200 text-indigo-700 text-[10px] font-bold flex items-center gap-1"><Layers className="w-3.5 h-3.5" /> إضافة خيار</button>
                </div>
                {variants.map((variant, index) => (
                  <div key={index} className="grid grid-cols-[.8fr_1.6fr_auto] gap-2 items-end">
                    <div><label className="block text-[10px] font-bold text-slate-600 mb-1">اسم الخيار</label><input value={variant.name} onChange={(e)=>setVariants((p)=>p.map((v,i)=>i===index?{...v,name:e.target.value}:v))} placeholder="اللون" className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs" /></div>
                    <div><label className="block text-[10px] font-bold text-slate-600 mb-1">القيم</label><input value={variant.options.join(', ')} onChange={(e)=>setVariants((p)=>p.map((v,i)=>i===index?{...v,options:e.target.value.split(',').map(x=>x.trim())}:v))} placeholder="أحمر, أزرق, أسود" className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs" /></div>
                    <button type="button" onClick={()=>setVariants((p)=>p.filter((_,i)=>i!==index))} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">حد تنبيه المخزون المنخفض</label>
                <input type="number" min={0} value={lowStockThreshold} onChange={(e)=>setLowStockThreshold(Number(e.target.value))} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs" />
              </div>
