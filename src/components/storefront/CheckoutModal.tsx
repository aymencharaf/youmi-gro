...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const getUnitPrice = (product: any, quantity: number) => {
    const tier = [...(product.tierPrices || [])]
      .filter((t: any) => t.minQuantity <= quantity && t.price > 0)
      .sort((a: any, b: any) => b.minQuantity - a.minQuantity)[0];
    return tier?.price ?? product.price;
  };

  const subtotal = cartItems.reduce((acc, item) => acc + getUnitPrice(item.product, item.quantity) * item.quantity, 0);
  ...
  const handleCompleteOrder = (e: React.FormEvent) => {
    ...
    const orderItems: OrderItem[] = cartItems.map((ci) => ({
      ...
      price: getUnitPrice(ci.product, ci.quantity),
