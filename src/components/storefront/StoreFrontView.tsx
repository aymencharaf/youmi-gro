...
export const StoreFrontView: React.FC<StoreFrontViewProps> = ({
  ...
  const cartSubtotal = cart.reduce((acc, item) => acc + getUnitPrice(item.product, item.quantity) * item.quantity, 0);
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FB] text-slate-800 flex flex-col dir-rtl font-['Tajawal']">
      ...
      {/* Main Store Header */}
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 px-4 lg:px-8 py-4 shadow-sm">
                  ...
                  <div key={idx} className="p-3 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-emerald-700 font-bold text-xs mt-0.5">
                        {getUnitPrice(item.product, item.quantity)} {store.currency}
                      </p>
                    </div>
