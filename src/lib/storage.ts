...
export function updateOrderStatus(storeSlug:string,orderId:string,newStatus:OrderStatus){const store=getStoreBySlug(storeSlug);if(!store)return null;const order=store.orders.find(o=>o.id===orderId);if(order)order.status=newStatus;saveStoreLocal(store);void api.updateOrder(orderId,newStatus).then(r=>{if(r.ok&&r.data?.store)saveStoreLocal(r.data.store);});return store;}
